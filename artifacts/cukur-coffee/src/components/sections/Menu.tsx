...
export default function Menu() {
  ...
  const { menuOverrides, nameOverrides, promos, hiddenItems, customItems } = useAdminData();

  const activeIndex = menuData.categories.indexOf(activeCategory);
  const rawItems = menuData.items.filter(
    (item) => item.category === activeCategory && !(hiddenItems ?? []).includes(item.name)
  );
